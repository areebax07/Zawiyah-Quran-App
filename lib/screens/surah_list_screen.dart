...
// FIX: Converted to StatefulWidget to handle search state
...
class _SurahListScreenState extends State<SurahListScreen> {
  ...
  List<MapEntry<int, Map<String, dynamic>>> _filteredSurahs = [];
  ...
  void initState() {
    ...
    _filteredSurahs = QuranData.surahNames.entries.toList();
  ...
  void _onSearchChanged() {
    ...
    setState(() {
      _filteredSurahs = QuranData.surahNames.entries.where((entry) {
        final surahInfo = entry.value;
        final englishName = (surahInfo['englishName'] as String).toLowerCase();
        final arabicName = (surahInfo['arabicName'] as String).toLowerCase();
        final surahNumber = entry.key.toString();
        return englishName.contains(query) || arabicName.contains(query) || surahNumber.contains(query);
      }).toList();
    });
  }
      ...
      builder: (context, settings, child) {
        ...
        return Scaffold(
          ...
          appBar: AppBar(
            ...
            elevation: 0,
            // FIX: Search bar toggles in app bar
            title: _isSearching
                ? TextField(
                ...
                : Text('SURAH', style: TextStyle(fontWeight: FontWeight.bold, color: textColor)),
            actions: [
              IconButton(
          ...
          body: CustomScrollView(
            slivers: [
              // Hide greeting and card when searching
              if (!_isSearching)
                SliverToBoxAdapter(
                  child: Padding(
              ...
              // FIX: List now uses the filtered surahs
              SliverList(
                delegate: SliverChildBuilderDelegate(
                  (context, index) {
                    final surahEntry = _filteredSurahs[index];
                    final surahNumber = surahEntry.key;
                    final surahInfo = surahEntry.value;

                    return Column(
                      children: [
                        ListTile(
                          leading: _buildNumberShape(surahNumber, purpleColor, textColor),
                          title: Text(surahInfo['englishName']! as String, style: TextStyle(color: textColor, fontWeight: FontWeight.bold)),
                          subtitle: Text("${(surahInfo['revelationType'] as String).toUpperCase()} - ${surahInfo['numberOfAyahs']} VERSES", style: TextStyle(color: textColor.withOpacity(0.7), fontSize: 12)),
                          trailing: Text(surahInfo['arabicName']! as String, style: TextStyle(fontSize: 20, color: purpleColor, fontFamily: 'MeQuran')),
                          onTap: () {
                            Provider.of<QuranProvider>(context, listen: false).setLastReadSurah({'number': surahNumber, ...surahInfo});
                            Navigator.push(context, MaterialPageRoute(builder: (context) => QuranPageScreen(surahNumber: surahNumber)));
                          },
                        ),
                        Padding(
  ...
  Widget _buildLastReadCard(BuildContext context) {
    ...
    return Container(
        ...
        decoration: BoxDecoration(
            gradient: const LinearGradient(colors: [Color(0xFFA060E2), Color(0xFF863ED5)], begin: Alignment.topLeft, end: Alignment.bottomRight),
          ...
          children: [
            Column(
              ...
              children: [
                ...
                Text(lastRead?['englishName'] as String? ?? "Al-Fatihah", style: const TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)),
                const SizedBox(height: 4),
                Text("Ayah No: ${lastRead?['ayahNumber'] ?? 1}", style: const TextStyle(color: Colors.white, fontSize: 14)),
              ],
            ),
